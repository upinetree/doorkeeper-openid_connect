...
module Doorkeeper
  module ClientAssertion
    # Validates a JWT client assertion for private_key_jwt client authentication.
    #
    # Implements the JWT client authentication profile defined in:
    #   RFC 7521 Section 4.2  - Using Client Authentication
    #   RFC 7523 Section 3    - JWT Format and Processing Requirements
    #   OpenID Connect Core 1.0 Section 9 - private_key_jwt client authentication
    class ClientAssertionValidator
      ...
      def verify_claims(decoded)
        raise JwtVerificationError, 'Missing required claims' unless verify_required_claims(decoded)
        raise JwtVerificationError, 'Issuer or subject mismatch' unless verify_issuer_and_subject(decoded)
        raise JwtVerificationError, 'Invalid issued_at time' unless verify_issued_at(decoded)
      end

      def verify_required_claims(decoded)
        # RFC 7523 Section 3 requires: iss, sub, aud, exp
        # OpenID Connect Core 1.0 Section 9 additionally requires: iat
      ...
      def verify_issuer_and_subject(decoded)
        ...
        # RFC 7523 Section 3: iss and sub MUST both equal the client_id
        # when the JWT is used for client authentication
      ...
      def verify_issued_at(decoded)
        # RFC 7523 Section 3: iat is not required, but OpenID Connect Core 1.0 Section 9 requires it.
        # We additionally reject JWTs with a future iat as a security measure against
        # pre-issued assertions, allowing clock skew tolerance.
      ...
      def allowed_algorithms
        Doorkeeper::ClientAssertion.configuration.client_assertion_algorithms
      end

      def exp_tolerance
        Doorkeeper::ClientAssertion.configuration.jwt_assertion_exp_tolerance
      end
    end
  end
end
