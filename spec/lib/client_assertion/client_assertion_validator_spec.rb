...
describe Doorkeeper::ClientAssertion::ClientAssertionValidator do
  ...
  describe '#valid?' do
    ...
    context 'with no public keys' do
      before { allow(application).to receive(:public_keys).and_return([]) }
    ...
    context 'with different ECDSA algorithms' do
      %w[ES256 ES384 ES512].each do |algorithm|
        context "when using #{algorithm}" do
          ...
          before do
            allow(Doorkeeper::ClientAssertion.configuration)
